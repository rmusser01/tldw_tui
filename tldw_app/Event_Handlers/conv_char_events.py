...
def get_selected_label_from_select(select_widget: Select) -> Optional[str]:
    """Gets the display label of the currently selected option in a Select widget."""
    if select_widget.value is Select.BLANK or select_widget.value is None:
        return None
    for label, value_option in select_widget.options: # Renamed value to value_option to avoid conflict
        if value_option == select_widget.value:
            return label
    return None

async def populate_ccp_character_select(app: 'TldwCli') -> None:
    """Populates the character selection dropdown in the CCP tab."""
    loguru_logger.info("Attempting to populate #conv-char-character-select dropdown.")
    if not app.notes_service:
        loguru_logger.error("Notes service not available, cannot populate character select for CCP tab.")
        try:
            ...
            loguru_logger.error("Failed to find #conv-char-character-select to show service error.")
        return

    try:
        db = app.notes_service._get_db(app.notes_user_id)
        character_cards = db.list_character_cards(limit=1000)  # Fetch a reasonable number
        ...
        if options:
            ...
            # Optionally set a default, e.g., the first character or leave blank
            # char_select_widget.value = options[0][1] # Set default to the first character's ID
            char_select_widget.prompt = "Select Character..."
            loguru_logger.info(f"Populated #conv-char-character-select with {len(options)} characters.")
        else:
            char_select_widget.set_options([("No characters found", Select.BLANK)])
            char_select_widget.value = Select.BLANK
            char_select_widget.prompt = "No characters available"
            loguru_logger.info("No characters found to populate #conv-char-character-select.")

    except QueryError as e_query:
        loguru_logger.error(f"Failed to find #conv-char-character-select widget: {e_query}", exc_info=True)
    except CharactersRAGDBError as e_db:
        loguru_logger.error(f"Database error populating #conv-char-character-select: {e_db}", exc_info=True)
        try:
            char_select_widget_err = app.query_one("#conv-char-character-select", Select) # Renamed to avoid scope collision
        ...
        loguru_logger.error(f"Unexpected error populating #conv-char-character-select: {e_unexp}", exc_info=True)
        try:
            char_select_widget_unexp = app.query_one("#conv-char-character-select", Select) # Renamed
...
async def populate_ccp_prompts_list_view(app: 'TldwCli', search_term: Optional[str] = None) -> None:
    ...
    if not app.prompts_service_initialized:
        try:
            list_view_prompt_err = app.query_one("#ccp-prompts-listview", ListView) # Renamed
            await list_view_prompt_err.clear()
            await list_view_prompt_err.append(ListItem(Label("Prompts service not available.")))
        except QueryError:
            loguru_logger.error("Failed to find #ccp-prompts-listview to show service error.")
        return

    try:
        list_view_prompt = app.query_one("#ccp-prompts-listview", ListView) # Renamed
        ...
        if search_term:
            results_tuple = prompts_interop.search_prompts(
                ...
                page=1, results_per_page=100,  # Fetch a good number
                include_deleted=False  # Usually you don't want deleted items in search
            )
        ...
        if not results:
            ...
            for prompt_data in results:
                ...
                # if version is not None: display_name += f" (v{version})"
                item = ListItem(Label(display_name))
                item.prompt_id = prompt_data.get('id')
                item.prompt_uuid = prompt_data.get('uuid')  # Store UUID as well if available
                await list_view_prompt.append(item)
        loguru_logger.info(f"Populated CCP prompts list. Search: '{search_term}', Found: {len(results)}")
    except QueryError as e_query:
        loguru_logger.error(f"UI component error populating CCP prompts list: {e_query}", exc_info=True)
    except (prompts_interop.DatabaseError,
            RuntimeError) as e_prompt_service:  # Catch specific errors from prompts_interop
        loguru_logger.error(f"Error populating CCP prompts list: {e_prompt_service}", exc_info=True)
        try:
            list_view_err_detail = app.query_one("#ccp-prompts-listview", ListView) # Renamed
            await list_view_err_detail.clear()
            await list_view_err_detail.append(ListItem(Label(f"Error loading prompts: {type(e_prompt_service).__name__}")))
        except QueryError: pass
    except Exception as e_unexp:
        loguru_logger.error(f"Unexpected error populating CCP prompts list: {e_unexp}", exc_info=True)
...
def clear_ccp_prompt_fields(app: 'TldwCli') -> None:
    ...
    try:
        ...
        # Reset reactive prompt details on the app instance
        app.current_prompt_id = None
        app.current_prompt_uuid = None
        app.current_prompt_name = None
        app.current_prompt_author = None
        app.current_prompt_details = None
        app.current_prompt_system = None
        app.current_prompt_user = None
        app.current_prompt_keywords_str = ""
        app.current_prompt_version = None
    except QueryError as e:
        loguru_logger.error(f"Error clearing CCP prompt fields: {e}", exc_info=True)

async def load_ccp_prompt_for_editing(app: 'TldwCli', prompt_id: Optional[int] = None, prompt_uuid: Optional[str] = None) -> None:
    ...
    if prompt_id is not None:
        ...
        # Use app's logger if available, otherwise global logging
    ...
    try:
        ...
        if prompt_details:
            ...
            app.current_prompt_keywords_str = ", ".join(prompt_details.get('keywords', []))
            ...
            app.query_one("#ccp-prompt-keywords-textarea", TextArea).text = app.current_prompt_keywords_str or ""
...
async def perform_ccp_conversation_search(app: 'TldwCli') -> None:
    ...
    try:
        ...
        if not app.notes_service:
            loguru_logger.error("Notes service not available for CCP conversation search.")
        ...
        if selected_character_id:
            loguru_logger.debug(f"CCP Search: Filtering for character ID: {selected_character_id}")
            if search_term:
                loguru_logger.debug(f"CCP Search: Term '{search_term}', CharID {selected_character_id}")
                conversations = db.search_conversations_by_title(
                    title_query=search_term, character_id=selected_character_id, limit=200
                )
            else:
                loguru_logger.debug(f"CCP Search: All conversations for CharID {selected_character_id}")
            ...
            loguru_logger.debug(f"CCP Search: No character selected. Global search with term: '{search_term}'")
            conversations = db.search_conversations_by_title(
        ...
        if not conversations:
            ...
            for conv_data in conversations:
                ...
                if char_id_for_conv:  # Try to prefix with character name
                    try:
                        # Check if this char_id is the one currently selected in the dropdown for optimization
                        char_name_prefix = ""
                        # Corrected logic for getting selected label:
                        if char_select_widget.value == char_id_for_conv:
                            selected_label = get_selected_label_from_select(char_select_widget)
                            if selected_label:
                                char_name_prefix = f"[{selected_label}] "
                        
                        if not char_name_prefix: # Fetch if not matched or no label from current select
                    ...
                    except Exception as e_char_name:
                        loguru_logger.warning(f"Could not fetch char name for conv {conv_data['id']}: {e_char_name}",
                                        exc_info=False)

                item = ListItem(Label(display_title))
                item.details = conv_data  # Store all conversation details from DB
        ...
        loguru_logger.error(f"UI component not found during CCP conversation search: {e_query}", exc_info=True)
        ...
        loguru_logger.error(f"Database error during CCP conversation search: {e_db}", exc_info=True)
        if 'results_list_view' in locals() and results_list_view.is_mounted:
            await results_list_view.clear()
            await results_list_view.append(ListItem(Label("Error: Database search failed.")))
    except Exception as e_unexp:
        loguru_logger.error(f"Unexpected error during CCP conversation search: {e_unexp}", exc_info=True)
        if 'results_list_view' in locals() and results_list_view.is_mounted:
            await results_list_view.clear()
...
async def handle_ccp_load_conversation_button_pressed(app: 'TldwCli') -> None:
    ...
    logging.info("CCP Load Conversation button pressed.")
    logger = getattr(app, 'loguru_logger', logging)
    try:
        results_list_view = app.query_one("#conv-char-search-results-list", ListView)
        ...
        if not (highlighted_item and hasattr(highlighted_item, 'details')):
            loguru_logger.warning("No conversation selected in CCP list or item has no details.")
            app.notify("Please select a conversation to load.", severity="warning")
            return

        conv_details_from_item = highlighted_item.details  # This is the full dict from DB
        loaded_conversation_id = conv_details_from_item.get('id')

        if not loaded_conversation_id:
            loguru_logger.error("Selected item in CCP list is missing conversation ID.")
            app.notify("Selected item is invalid.", severity="error")
            return

        app.current_conv_char_tab_conversation_id = loaded_conversation_id  # Reactive variable
        loguru_logger.info(f"Current CCP tab conversation ID set to: {loaded_conversation_id}")

        # --- Populate Right Pane (Conversation Details) ---
        ...
        if not app.notes_service:
            loguru_logger.error("Notes service not available for loading CCP messages.")
            await center_pane.mount(Static("Error: Notes service unavailable for messages."))
            return

        # Use the app's _load_branched_conversation_history method, which should exist in app.py
        await app._load_branched_conversation_history(loaded_conversation_id, center_pane)

        center_pane.scroll_end(animate=False)
        loguru_logger.info(f"Loaded messages into #conv-char-center-pane for conversation {loaded_conversation_id}.")
        app.notify(f"Conversation '{title_input_ccp.value}' loaded.", severity="information")

    except QueryError as e_query:
        loguru_logger.error(f"UI component not found during CCP load conversation: {e_query}", exc_info=True)
        app.notify("Error: UI component missing for loading.", severity="error")
    except CharactersRAGDBError as e_db:
        loguru_logger.error(f"Database error during CCP load conversation: {e_db}", exc_info=True)
        app.notify("Error loading conversation data from database.", severity="error")
    except Exception as e_unexp:
        loguru_logger.error(f"Unexpected error during CCP load conversation: {e_unexp}", exc_info=True)
...
async def handle_ccp_save_conversation_details_button_pressed(app: 'TldwCli') -> None:
    ...
    logging.info("CCP Save Conversation Details button pressed.")
    logger = getattr(app, 'loguru_logger', logging)
    if not app.current_conv_char_tab_conversation_id:
        loguru_logger.warning("No current conversation loaded in CCP tab to save details for.")
        app.notify("No conversation loaded to save details for.", severity="warning")
        return

    if not app.notes_service:
        loguru_logger.error("Notes service is not available for CCP save details.")
    ...
    try:
        ...
        if not current_conv_details_ccp:
            loguru_logger.error(f"CCP: Conversation {target_conversation_id_ccp} not found in DB for saving details.")
        ...
        if current_db_version_ccp is None:
            loguru_logger.error(f"CCP: Conversation {target_conversation_id_ccp} is missing version information.")
        ...
        if new_title_ccp != current_conv_details_ccp.get('title', ''):
            ...
            current_db_version_ccp += 1  # Increment version for subsequent keyword ops on this save
            await perform_ccp_conversation_search(app)  # Refresh list in left pane if title changed
        ...
        for kw_text_add in keywords_to_add_ccp:
            kw_id_add = db.add_keyword(kw_text_add) 
            if kw_id_add: # add_keyword returns int ID
        ...
        if title_updated_ccp or keywords_changed_ccp:
            loguru_logger.info(f"CCP: Details saved for conversation {target_conversation_id_ccp}.")
            app.notify("Details saved successfully!", severity="information")
            final_keywords_list_ccp = db.get_keywords_for_conversation(
                target_conversation_id_ccp)  # Re-fetch to get canonical casing
        ...
        loguru_logger.error(f"CCP: UI component not found for saving details: {e_query}", exc_info=True)
        app.notify("Error accessing UI fields.", severity="error")
    except CharactersRAGDBError as e_db:
        loguru_logger.error(f"CCP: Database error saving details: {e_db}", exc_info=True)
        app.notify("Database error saving details.", severity="error")
    except Exception as e_unexp:
        loguru_logger.error(f"CCP: Unexpected error saving details: {e_unexp}", exc_info=True)
...
async def handle_ccp_prompt_create_new_button_pressed(app: 'TldwCli') -> None:
    ...
    logging.info("CCP Create New Prompt button pressed.")
    logger = getattr(app, 'loguru_logger', logging)
    clear_ccp_prompt_fields(app)
    try:
        # Set defaults for a new prompt
        app.query_one("#ccp-prompt-name-input", Input).value = "New Prompt"
        ...
        loguru_logger.error(f"CCP: UI error preparing for new prompt: {e_query}", exc_info=True)
...
async def handle_ccp_prompt_load_selected_button_pressed(app: 'TldwCli') -> None:
    ...
    logging.info("CCP Load Selected Prompt button pressed.")
    logger = getattr(app, 'loguru_logger', logging)
    try:
        ...
        app.notify(f"Error loading prompt: {e}", severity="error")
        loguru_logger.error(f"Error loading prompt from list: {e}", exc_info=True)


async def handle_ccp_prompt_save_button_pressed(app: 'TldwCli') -> None:
    """Handles saving a new or existing prompt in the CCP tab."""
    logging.info("CCP Save Prompt button pressed.")
    logger = getattr(app, 'loguru_logger', logging)
    if not app.prompts_service_initialized:
        app.notify("Prompts service not available.", severity="error")
        return
    ...
    try:
        ...
        if app.current_prompt_id is None:  # Creating a new prompt
            loguru_logger.info(f"CCP: Saving new prompt: {name}")
            # add_prompt (name, author, details, system_prompt, user_prompt, keywords, client_id, overwrite=False)
            # Returns: (Optional[int], Optional[str], str) -> (prompt_id, prompt_uuid, message)
            saved_id, saved_uuid, message = prompts_interop.add_prompt(
                name=name, author=author, details=details,
                system_prompt=system_prompt, user_prompt=user_prompt,
                keywords=keywords_list, client_id=app.prompts_client_id,  # Add client_id
                overwrite=False  # Explicitly False for a "new" save from UI
            )
        else:  # Updating an existing prompt
            loguru_logger.info(f"CCP: Updating prompt ID: {app.current_prompt_id}, Name: {name}")
            update_payload = {
                'name': name, 'author': author, 'details': details,
                'system_prompt': system_prompt, 'user_prompt': user_prompt,
                'keywords': keywords_list
            }
            # update_prompt_by_id(prompt_id: int, update_data: Dict[str, Any], client_id: str)
            # Returns: (Optional[str], str) -> (new_uuid_if_changed_or_current, message)
            # The Prompts_DB_v2's update_prompt_by_id does versioning internally.
            updated_uuid, message = prompts_interop.get_db_instance().update_prompt_by_id(
                prompt_id=app.current_prompt_id,
                update_data=update_payload,
                client_id=app.prompts_client_id  # Add client_id
            )
            saved_id = app.current_prompt_id  # ID doesn't change on update
        ...
        if saved_id is not None or saved_uuid is not None:
            ...
            # Reload the saved/updated prompt to get latest state
            await load_ccp_prompt_for_editing(app, prompt_id=saved_id, prompt_uuid=saved_uuid)
        else:
            # This case means add_prompt or update_prompt_by_id returned (None, None, message) or (None, message)
            app.notify(f"Failed to save prompt: {message or 'Unknown error'}", severity="error")
        ...
        loguru_logger.error(f"CCP Save Prompt: UI component error: {e_query}", exc_info=True)
        app.notify("UI Error saving prompt.", severity="error")
    except Exception as e_save:
        loguru_logger.error(f"CCP: Error saving prompt: {e_save}", exc_info=True)
...
async def handle_ccp_prompt_clone_button_pressed(app: 'TldwCli') -> None:
    ...
    logging.info("CCP Clone Prompt button pressed.")
    logger = getattr(app, 'loguru_logger', logging)
    ...
    try:
        ...
        cloned_name = f"Clone of {original_name} ({timestamp})"[:100]  # Ensure reasonable length

        # Use reactive values as the source for cloning
        cloned_id, cloned_uuid, msg = prompts_interop.add_prompt(
            name=cloned_name,
            author=app.current_prompt_author or "", #  ensure string
            details=f"Clone of: {app.current_prompt_details}",
            system_prompt=app.current_prompt_system,
            user_prompt=app.current_prompt_user,
            keywords=[kw.strip() for kw in app.current_prompt_keywords_str.split(',') if kw.strip()],
            client_id=app.prompts_client_id,  # Add client_id
            overwrite=False  # A clone should be a new entry, name is made unique
        )
        if cloned_id:
            app.notify(f"Prompt cloned as '{cloned_name}'. {msg}", severity="information")
            ...
            app.notify(f"Failed to clone prompt: {msg}", severity="error")
    except Exception as e_clone:
        loguru_logger.error(f"CCP: Error cloning prompt: {e_clone}", exc_info=True)
...
async def handle_ccp_prompt_delete_button_pressed(app: 'TldwCli') -> None:
    ...
    logging.info("CCP Delete Prompt button pressed.")
    logger = getattr(app, 'loguru_logger', logging)
    if not app.prompts_service_initialized or app.current_prompt_id is None:
        app.notify("No prompt loaded to delete or service unavailable.", severity="warning")
        return

    # TODO: Implement a confirmation dialog (future step)
    # For now, direct delete:
    try:
        success = prompts_interop.soft_delete_prompt(app.current_prompt_id,
                                                     client_id=app.prompts_client_id)  # Add client_id
        if success:
            app.notify(f"Prompt '{app.current_prompt_name}' deleted.", severity="information")
            clear_ccp_prompt_fields(app)  # This also resets reactive vars
            await populate_ccp_prompts_list_view(app)
            app.query_one("#ccp-prompt-details-collapsible", Collapsible).collapsed = True
        else:
            # This might happen if the prompt was already deleted or some other DB constraint failed silently
            app.notify(f"Failed to delete prompt '{app.current_prompt_name}'. It might have been already deleted.",
                       severity="error", timeout=7)
    except prompts_interop.NotFoundError:
        app.notify(f"Prompt '{app.current_prompt_name}' not found for deletion.", severity="error")
    except prompts_interop.DatabaseError as e_db_del:
        loguru_logger.error(f"CCP: Database error deleting prompt: {e_db_del}", exc_info=True)
        app.notify(f"Database error deleting prompt: {type(e_db_del).__name__}", severity="error")
    except Exception as e_del:
        loguru_logger.error(f"CCP: Error deleting prompt: {e_del}", exc_info=True)
        app.notify(f"Error deleting prompt: {type(e_del).__name__}", severity="error")


# --- Input/List View Changed Handlers for CCP Tab ---
...
async def handle_ccp_character_select_changed(app: 'TldwCli', selected_value: Any) -> None:
    ...
    loguru_logger.debug(f"CCP Character Select changed to: {selected_value}")
    # When character selection changes, re-filter the conversation list
    await perform_ccp_conversation_search(app)
...
async def handle_ccp_prompts_list_view_selected(app: 'TldwCli', list_view_id: str, item: Any) -> None:
    ...
    if item and (hasattr(item, 'prompt_id') or hasattr(item, 'prompt_uuid')):
        ...
        loguru_logger.info(f"CCP Prompt selected from list: ID={prompt_id_to_load}, UUID={prompt_uuid_to_load}")
        await load_ccp_prompt_for_editing(app, prompt_id=prompt_id_to_load, prompt_uuid=prompt_uuid_to_load)
    else:
        loguru_logger.debug("CCP Prompts ListView selection was empty or item lacked ID/UUID.")
