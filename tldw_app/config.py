...
def load_settings() -> Dict:
    ...
    logging_section = get_toml_section('Logging')
    ...
    log_level_toml = _get_typed_value(logging_section, "log_level", log_level_env, str).upper()
    ...
    config_dict = {
        ...
        "chunking_config": {
            ...
            # ... (add all other chunking types: audio, book, document, mediawiki_article, etc.)
        },
        "embedding_config": {
...
# --- Python dictionary representation of the default configuration ---
# This is generated by parsing the CONFIG_TOML_CONTENT above.
# Your app.py will use this as a fallback if config.toml is missing or incomplete.
...
def load_config(force_reload: bool = False) -> Dict[str, Any]:
    """
    Loads configuration from TOML file, falling back to DEFAULT_CONFIG.
    ...
    if not DEFAULT_CONFIG_PATH.exists():
        logging.info(f"Config file not found at {DEFAULT_CONFIG_PATH}. Using default configuration.")
        # Optionally, create the default config file here
        try:
            DEFAULT_CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
            with open(DEFAULT_CONFIG_PATH, "w", encoding="utf-8") as f:
                f.write(CONFIG_TOML_CONTENT)
            logging.info(f"Created default config file at {DEFAULT_CONFIG_PATH}")
        except OSError as e:
            logging.error(f"Could not create default config file {DEFAULT_CONFIG_PATH}: {e}")
    else:
        try:
            ...
            logging.error(f"An unexpected error occurred while loading {DEFAULT_CONFIG_PATH}: {e}. Using default configuration.")
            # loaded_config remains as DEFAULT_CONFIG
...
def get_setting(section: str, key: str, default: Any = None) -> Any:
    """Gets a specific setting from the loaded configuration."""
    config = load_config()
    ...
    else: # Allow for None to be a valid configured value
        return config.get(section, {}).get(key)


# --- Function to get providers and their models ---
def get_providers_and_models() -> Dict[str, List[str]]:
    """
    Loads provider and model configuration from the [providers] section,
    validates it, and returns a dictionary of valid providers mapped
    to their list of models.
    """
    config = load_config() # Uses the CLI-specific load_config
    providers_data = config.get("providers", {}) # Get the [providers] table
    ...
    if isinstance(providers_data, dict):
        for provider, models in providers_data.items():
            if isinstance(models, list) and all(isinstance(m, str) for m in models):
                ...
                logging.warning(
                    f"Invalid model list for provider '{provider}' in config's [providers] section. "
                    f"Value: {models!r}. Expected a list of strings. Skipping."
                )
    else:
        logging.error(
            f"Config's 'providers' section is not a dictionary: {providers_data!r}. "
            f"No provider/model data available from [providers] section."
        )
        # Fallback to an empty dict if DEFAULT_CONFIG itself had a bad "providers" type,
        # though tomllib.loads(CONFIG_TOML_CONTENT) should ensure it's a dict.
        if not isinstance(DEFAULT_CONFIG.get("providers"), dict):
             logging.error("DEFAULT_CONFIG 'providers' section is also not a dict. This is a bug in DEFAULT_CONFIG.")
             return {} # Hard fallback

    logging.debug(f"get_providers_and_models returning providers: {list(valid_providers.keys())}")
    return valid_providers

# --- Database and Log File Path (from your initial block, adapted to use the above) ---
def get_database_path() -> Path:
    config = load_config()  # Ensures config is loaded
    # Use get_setting to access the path, falling back to DEFAULT_CONFIG if necessary
    default_db_path = DEFAULT_CONFIG.get("database", {}).get("path", "~/.local/share/tldw_cli/tldw_cli_data.db")
    db_path_str = get_setting("database", "path", default_db_path)
